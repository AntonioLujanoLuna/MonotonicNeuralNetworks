...
from src.PWLNetwork import pwl, pwl_mono_reg
...
def train_model(model: nn.Module, optimizer: AdamWScheduleFree, train_loader: DataLoader,
    ...
    for epoch in range(max_epochs):
        ...
        for batch_X, batch_y in train_loader:
            ...
            # Use batch_X if mono_reg_instances is 0, otherwise sample from the loader
            mono_X = batch_X if mono_reg_instances == 0 else sample_from_loader(train_loader, mono_reg_instances,
                                                                                device)
            ...
            def closure():
                ...
                monotonicity_loss = pwl_mono_reg(model, mono_X, monotonic_indices, config["offset"])
...
def evaluate_model(model: nn.Module, optimizer: AdamWScheduleFree, data_loader: DataLoader,
    ...
    for batch_X, batch_y in data_loader:
        ...
        mono_X = batch_X if mono_reg_instances == 0 else sample_from_loader(data_loader, mono_reg_instances, device)
        with torch.enable_grad():
            monotonicity_loss = pwl_mono_reg(model, mono_X, monotonic_indices, offset)
...
def process_dataset(data_loader: Callable, results_file: str) -> None:
    ...
    for weight in mono_weights:
        ...
        current_config["offset"] = 0.  # Add offset to the config
