...
from src.CertifiedMonotonicNetworks import uniform_pwl, CertifiedMonotonicNetwork
...
def create_model(config: Dict, input_size: int, task_type: str, monotonic_indices: List[int], seed: int) -> CertifiedMonotonicNetwork:
    torch.manual_seed(seed)
    output_activation = nn.Identity() if task_type == "regression" else nn.Sigmoid()
    return CertifiedMonotonicNetwork(
...
def train_model(model: CertifiedMonotonicNetwork, optimizer: AdamWScheduleFree, train_loader: DataLoader, val_loader: DataLoader,
    ...
    for epoch in range(config["epochs"]):
        ...
        for batch_X, batch_y in train_loader:
            ...
            optimizer.zero_grad()
            loss = uniform_pwl(model, optimizer, batch_X, batch_y, task_type, monotonic_indices,
...
def optimize_hyperparameters(X: np.ndarray, y: np.ndarray, task_type: str, monotonic_indices: List[int],
    ...
    def objective(trial):
        ...
        model = create_model(config, X.shape[1], task_type, monotonic_indices, GLOBAL_SEED).to(device)
