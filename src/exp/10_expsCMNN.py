...
def train_model(model: nn.Module, optimizer: AdamWScheduleFree, train_loader: torch.utils.data.DataLoader,
    ...
    for epoch in range(max_epochs):
        ...
        for batch_X, batch_y in train_loader:
            ...
            _ = uniform_pwl(model, optimizer, batch_X, batch_y, task_type, monotonic_indices,
...
def evaluate_model(model: nn.Module, optimizer: AdamWScheduleFree, data_loader: DataLoader,
                   ...
                   monotonicity_weight: float, regularization_budget: int = 1024, b: float = 0.2) -> Tuple[float, float]:
    model.eval()
    optimizer.eval()
    ...
    for batch_X, batch_y in data_loader:
        ...
        # Calculate monotonicity loss using uniform_pwl approach
        monotonicity_loss = torch.tensor(0.0, device=batch_X.device)
        if monotonic_indices:
            # Generate regularization points
            reg_points = torch.rand(regularization_budget, batch_X.shape[1], device=device)
        ...
        # Combine losses as per the equation in uniform_pwl
...
def main():
    ...
    results_file = "expsuniform_PWL.csv"
