...
from src.MonoNetNetwork import MonoNet
...
def create_model(config: Dict, input_size: int, seed: int) -> MonoNet:
    torch.manual_seed(seed)
    return MonoNet(
        num_features=input_size,
        num_classes=1,
        hidden_sizes=config["hidden_sizes"],
        interpretable_size=config["interpretable_size"],
        monotonic_sizes=config["monotonic_sizes"],
        activation=nn.Tanh(),
    )
...
def objective(trial: optuna.Trial, dataset: TensorDataset, train_dataset: torch.utils.data.Subset,
    ...
    hidden_sizes_options = generate_layer_combinations(min_layers=1, max_layers=3, units=[8, 16, 32])
    monotonic_sizes_options = generate_layer_combinations(min_layers=1, max_layers=2, units=[8, 16, 32])

    config = {
        "lr": trial.suggest_float("lr", 1e-4, 1e-1, log=True),
        "hidden_sizes": ast.literal_eval(trial.suggest_categorical("hidden_sizes", hidden_sizes_options)),
        "interpretable_size": trial.suggest_int("interpretable_size", 4, 16),
        "monotonic_sizes": ast.literal_eval(trial.suggest_categorical("monotonic_sizes", monotonic_sizes_options)),
