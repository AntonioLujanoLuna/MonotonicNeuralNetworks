...
def create_model(config: Dict, input_size: int, seed: int, monotonic_indices: List[int]) -> ScalableMonotonicNeuralNetwork:
    torch.manual_seed(seed)
    return ScalableMonotonicNeuralNetwork(
        input_size=input_size,
        mono_feature=monotonic_indices,
        exp_unit_size=tuple(config["unit_size"] for _ in range(config["n_layers"])),
        relu_unit_size=tuple(config["unit_size"] for _ in range(config["n_layers"])),
        conf_unit_size=tuple(config["unit_size"] for _ in range(config["n_layers"]))
    )
