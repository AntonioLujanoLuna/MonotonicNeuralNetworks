...
def create_model(config: Dict, input_size: int, seed: int) -> WeightsConstrainedMLP:
    torch.manual_seed(seed)
    # output_activation = nn.Identity() if task_type == "regression" else nn.Sigmoid() to enforce stability use logits
