...
from src.PartialMonotonicNetwork import PartialMonotonicNetwork
...
def objective(trial: optuna.Trial, dataset: TensorDataset, train_dataset: torch.utils.data.Subset,
    ...
    config = {
        ...
        "mono_hidden_sizes": ast.literal_eval(
            trial.suggest_categorical("mono_hidden_sizes", mono_hidden_sizes_options)),
        "non_mono_hidden_sizes": ast.literal_eval(
...
def optimize_hyperparameters(X: np.ndarray, y: np.ndarray, task_type: str, monotonic_indices: List[int], n_trials: int = 30,
    ...
    try:
        study.optimize(
            lambda trial: objective(trial, dataset, train_dataset, val_dataset, task_type, monotonic_indices),
