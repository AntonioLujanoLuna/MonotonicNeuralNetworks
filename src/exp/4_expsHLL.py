...
def process_fold(fold_data, best_config, task_type, monotonic_indices, GLOBAL_SEED):
    ...
    model = create_model(best_config, X.shape[1], task_type, GLOBAL_SEED, monotonic_indices, device)
...
def process_repeat(repeat_data, best_config, task_type, monotonic_indices, GLOBAL_SEED):
    ...
    model = create_model(best_config, X_train.shape[1], task_type, GLOBAL_SEED, monotonic_indices, device)
    n_params = count_parameters(model)
...
def main():
    ...
    dataset_loaders = [
        ...
        load_compas, load_era, load_esl, load_heart, load_swd,
    ]

    dataset_loaders = [
        load_blog_feedback, load_lev, load_auto_mpg, load_abalone, load_boston_housing,
        load_compas, load_era, load_esl, load_heart, load_swd
    ] # No loan - too much time
